"""
Phase 2: Regenerate ALL paper figures with Chinese labels, better layout,
Wilson CI error bars, split multi-panel figures. Each case in §5.1 gets
its own individual figure. Ablation is simplified.
"""
...
FIG_DIR = R_DIR / "paper_figures" / "v4"
FIG_DIR.mkdir(parents=True, exist_ok=True)

CASE_NAMES = {
    "poisson": "泊松方程", "burgers": "伯格斯方程",
    "stokes_poiseuille": "斯托克斯-泊肃叶流", "fisher_kpp": "费希尔方程",
}

def pivot_df(df, vcol):
    return df.pivot(index="noise_std", columns="num_observation", values=vcol).sort_index().sort_index(axis=1)

# ═══ FIG 5-1: Individual case rel_l2 phase maps (4 separate figures) ═══

def gen_fig5_1():
    """Generate 4 individual rel_l2 phase maps, one per case."""
    ...
    for c in ["poisson","stokes_poiseuille","fisher_kpp","burgers"]:
        ...
        for i in range(len(t.index)):
            for j in range(len(t.columns)):
                ax.text(j, i, f"{t.values[i,j]:.3f}", ha="center", va="center", color="white", fontsize=6)
...
# ═══ FIG 5-3: Probability boundary heatmaps (3 individual) ═══

def gen_fig5_3():
    srcs = [
        (R_DIR/"probability_matrices"/"stokes_probability_boundary_v1"/"multiseed_summary.csv","斯托克斯-泊肃叶流","a"),
        (R_DIR/"probability_matrices"/"fisher_kpp_probability_boundary_v1"/"multiseed_summary.csv","费希尔方程","b"),
        (R_DIR/"probability_matrices"/"burgers_probability_boundary_v2_5seed"/"multiseed_summary.csv","伯格斯方程","c"),
    ]
...
# ═══ FIG 5-4: Wilson CI boundary keypoints plot (NEW - statistical) ═══

def gen_fig5_4():
    """Wilson confidence interval plot for Burgers + Fisher-KPP boundary keypoints."""
    # Load probe data
    probes = {}
    ...
    for ax, (case, name) in zip(axes, [("burgers","伯格斯方程"),("fisher_kpp","费希尔方程")]):
        if case not in probes: continue
        df = probes[case]
        # Group by label, compute failure rate and Wilson CI
    ...
    fp = FIG_DIR / "fig5-4.png"
    fig.savefig(fp, dpi=220, bbox_inches="tight"); plt.close(fig)
    print(f"  Fig 5-4: {fp}")

# ═══ FIG 6-1: Simplified ablation (just Full 4D vs rel_l2 vs minus-training) ═══

def gen_fig6_1():
    """Simplified ablation: for each case, show only Full 4D, 1D rel_l2, and the most-impactful removal."""
    sp = R_DIR / "analysis" / "dimension_ablation_v2" / "ablation_summary.json"
    if not sp.exists(): return
    data = json.loads(sp.read_text(encoding="utf-8"))
    
    cases = [("burgers","伯格斯方程"), ("fisher_kpp","费希尔方程"), ("stokes_poiseuille","斯托克斯-泊肃叶流")]
    keys_show = [
        ("R_full","完整四维","#333333"),
        ("rel_l2","仅相对误差","#b64040"),
    ...
    for ax, (ck, cn) in zip(axes, cases):
    ...
    axes[0].set_ylabel("平均斯皮尔曼相关系数", fontsize=11)
    fig.suptitle("消融对比：完整四维 R 与简化指标", fontsize=13)
    fp = FIG_DIR / "fig6-1.png"
    fig.savefig(fp, dpi=220, bbox_inches="tight"); plt.close(fig)
    print(f"  Fig 6-1: {fp}")

# ═══ FIG 7-1: Calibration sensitivity ═══
...
def gen_fig7_1():
    ...
    fig, axes = plt.subplots(1, len(cases), figsize=(4.5*len(cases), 3.5), constrained_layout=True)
...
def gen_fig7_3():
    """Safe-point failure rate vs threshold multiplier."""
    ...
    ax.set_title(f"伯格斯方程：最安全点越界率随阈值变化（{len(rows)}个种子）", fontsize=12)
...
def main():
    print("Phase 2: Regenerating figures...")
    gen_fig5_1()
    gen_fig5_2()
    gen_fig5_3()
    gen_fig5_4()
