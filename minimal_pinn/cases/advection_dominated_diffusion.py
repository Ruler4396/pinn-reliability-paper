...
class AdvectionDominatedDiffusionCase(BaseCase):
    name = "advection_dominated_diffusion"
    input_dim = 2
    output_dim = 1
    ...
    def __init__(
        ...
        # Stable scalar for normalization; eps is strictly positive in this study.
        self._exp_denom = 1.0 - math.exp(-1.0 / self.eps)
    ...
    def truth(self, x: torch.Tensor) -> torch.Tensor:
        xx = x[:, 0:1]
        yy = x[:, 1:2]
        # Boundary-layer profile concentrated near x=1, modulated by a y profile.
        layer = (1.0 - torch.exp((xx - 1.0) / self.eps)) / self._exp_denom
        return torch.sin(math.pi * yy) * layer
    ...
    def forcing(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (self.beta_x * u_x + self.beta_y * u_y - self.eps * (u_xx + u_yy)).detach()
    ...
    def physics_residual(self, x: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
        ...
        return self.beta_x * u_x + self.beta_y * u_y - self.eps * (u_xx + u_yy) - self.forcing(x)
